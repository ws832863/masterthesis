...
In this model the scale function $v(x)$ is estimated from $|r_t|^d$  with d=1, instead of ob-tained by taking the square root of $\hat{•v}^2(x)$ , which is estimated from $r_t^2$ . The reason of d=1 is that ensure the existence of the fourth order moment of $\varepsilon_{t}$. However, the scale function estimated from $|r_t|$ is an estimate of another function rather than v(x). Denote the estimated function g(x). Let $c_1=E(|\varepsilon_{t}|)\neq 1$ . Then we have  $g(x)=c_1v(x)$ and $v(x)=g(x)/c_1$ . As follows the local linear estimation is used to estimate the scale function.
...
The optimal bandwidth for estimating $g(x)$ is different to that for estimating $v^2(x)$   from $r_t^2$ . In this method a fully data-driven algorithm is used, where the iterative plug-in idea, which is introduced above, adapted to selected b in the Q function with a starting bandwidth, which is selected by the cross-validation method.
Sometimes, the local linear estimator may result in negative estimation, when sample size is small and a small bandwidth is selected. To ensure the non-negativity, the final estimator  $\hat{g}_f(x)$ is assumed to be $\hat{g}_f(x)=|\hat{g}(x,d)|$ . Let $\hat{\varepsilon}_{1,t}=r_t/\hat{g}_f(x)\approx c^{-1}\hat{\varepsilon}$  and $E(\varepsilon_t^4)$ is assumed to be finite. Then the consistent estimate of   is

\begin{equation}
\hat{c}_1=[\frac{1}{n}\sum_{t=1}^n\hat{\varepsilon}_{1,t}^2]^{-1/2}.
\end{equation}

Then the estimate of $v(x)$ can be obtained by

\begin{equation}
\hat{v}(x_t)=\hat{c}_1^{-1}\hat{g}_f(x_t).
\end{equation}

Then the estimate of $\varepsilon_t$ can defined as $\hat{\varepsilon}_t=r_t/\hat{v}(x_t)$ . The unknown parameters of a chosen APARCH model can be estimated by using the approximate conditional maxi-mal likelihood method. The method AIC or BIC can be used to select a suitable model \citep{}.
